...
# Column 1: Metrics labels (only labels)
with col_metrics:
    st.header("📍 Metrics")
    metrics_list = [
        "Monthly Traffic",
        ...
        "Average Lifetime Value per Lead ($)",
...
lifetime_revenue = lifetime_leads_captured * (avg_lifetime_value * lifetime_years)
...
| 🎯 **Revenue with Ad Grant**               | `${ad_grant_revenue:,.2f}`             |
| 🚀 **Revenue from Paid Ads**               | `${paid_ads_revenue:,.2f}`             |
| 💎 **Lifetime Revenue ({lifetime_years} yrs)** | `${lifetime_revenue:,.2f}`             |
